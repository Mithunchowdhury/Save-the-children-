                    ...
                    <div id="divSearch" runat="server" class="baseFormBodyFlex" style="width: 98%; margin-bottom: 10px; float: left;">
                        ...
                        <div style="font-size: 12px; clear: both; margin-top: -5px;">
                            ...
                            <div style="text-align: right; padding-right: 6px; padding-top: 5px;">
                                ...
                                <asp:Button ID="cmdSearch" runat="server" BorderStyle="None" Text="SEARCH" Width="120px" alt="ajax"
                                    CssClass="buttonStyle" OnClick="cmdSearch_Click" />
                            </div>
                        </div>
                    </div>
                    
                    <div style="clear: both;"></div>
                    <%--OnItemCommand="rgTaskManager_ItemCommand"--%>
                    <telerik:RadGrid ID="rgTaskManager" runat="server"   
...
<%--                                <telerik:GridBoundColumn FilterControlAltText="PRNO" UniqueName="PRNO" DataField="PRNO" Display="false">
    ...
    <script type="text/javascript">        
        $(window).load(function () {            
            Sys.WebForms.PageRequestManager.getInstance().add_pageLoaded(afterPageLoad1);
        });
        function afterPageLoad1(sender, args) {            
            var inputs, index;
            inputs = document.getElementsByTagName('input');
            if (inputs !== undefined) {
                for (index = 0; index < inputs.length; ++index) {

                    if (inputs[index].type !== undefined && inputs[index].type == 'radio') {
                        inputs[index].alt = 'ajax';
                    }
                    //else if (inputs[index].type !== undefined && inputs[index].type == 'submit' && 
                    //    inputs[index].id.indexOf("cmdView") > -1) {
                    //    inputs[index].alt = 'ajax';
                    //}
                    //else if (inputs[index].type !== undefined && inputs[index].type == 'submit' &&
                    //    inputs[index].id.indexOf("cmdManage") > -1) {
                    //    inputs[index].alt = 'ajax';
                    //}
                }
            }
